...
class BaseDescriptor(BaseModel):
    ...
    icon: Optional[str] = Field(
        None,
        description="Icon identifier for frontend presentation UI (e.g. 'Folder', 'Video', 'Calendar')."
    )
    accent_color: Optional[str] = Field(
        None,
        description="Color token for UI rendering (e.g. 'accent-memory', 'accent-video', 'accent-concept')."
    )
    availability_state: Optional[str] = Field(
        "available",
        description="Availability state for presentation UI (e.g. 'available', 'coming_soon', 'disabled')."
    )
...
class ProviderDescriptor(BaseDescriptor):
    ...
    capabilities: List[str] = Field(
        default_factory=list,
        description="Supported operations (e.g. 'sync', 'watch', 'authenticate')."
    )
    config_schema: Dict[str, Any] = Field(
        default_factory=dict,
        description="JSON Schema for configuration properties required by the provider."
    )
    permissions: List[str] = Field(
        default_factory=list,
        description="List of composable permission capabilities requested by the provider."
    )
    supported_types: List[str] = Field(
        default_factory=list,
        description="List of raw semantic types emitted by the provider."
    )
