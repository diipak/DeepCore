...
from deepcore2.runtime.descriptors.base import BaseDescriptor, DescriptorCategory
from deepcore2.runtime.descriptors.registry import CapabilityRegistry
from deepcore2.runtime.descriptors.exceptions import DescriptorNotFoundError
...
class CapabilitySummary(BaseModel):
    ...
    availability_state: Optional[str] = "available"
...
class CapabilityDiscoveryService:
    ...
    def _map_to_summary(self, desc: BaseDescriptor) -> CapabilitySummary:
        ...
        from deepcore2.runtime.descriptors import DescriptorCategory
        from deepcore2.core.acquisition.base import SyncContext
        
        avail = desc.availability_state
        
        if desc.category == DescriptorCategory.PROVIDER:
            app = get_application()
            sdk_present = (app.acquisition_manager is not None and desc.id in app.acquisition_manager._registry)
            wiring_present = (app.acquisition_runtime is not None)
            
            if sdk_present and wiring_present:
                try:
                    connector_cls = app.acquisition_manager.get_connector_class(desc.id)
                    connector = connector_cls()
                    test_ctx = SyncContext(
                        workspace_id=1,
                        source_id=0,
                        config={},
                        credentials={}
                    )
                    health_status = connector.health(test_ctx)
                    h_msg = (health_status.message or "").lower()
                    if health_status.state == "CRITICAL" and ("not installed" in h_msg or "unsupported" in h_msg or "missing on this os" in h_msg):
                        avail = "disabled"
                    else:
                        avail = "available"
                except Exception:
            ...
            if not avail:
                avail = "available" if (desc.enabled and desc.configurable) else "coming_soon"
