...
from deepcore2.storage.sqlite.models import RegistryObject as DBRegistryObject, SyncRun as DBSyncRun
from deepcore2.core.objects.schemas import RegistryObjectCreate, RegistryObjectUpdate, SyncRunCreate
from deepcore2.core.utils.text import extract_search_keywords
...
class RegistryService:
    ...
    def is_cancelled(self) -> bool:
        if self.run_uuid:
            run = self.db.query(DBSyncRun).filter(DBSyncRun.uuid == self.run_uuid).first()
            if run and run.status == "cancelled":
                return True
        return False

    def _get_query(self, object_id_or_uuid: Union[int, str]):
        """Helper to query a registry object by integer ID or string UUID, scoped to the workspace."""
        base_query = self.db.query(DBRegistryObject).filter(DBRegistryObject.workspace_id == self.workspace_id)
        if isinstance(object_id_or_uuid, int):
            return base_query.filter(DBRegistryObject.id == object_id_or_uuid)
        elif isinstance(object_id_or_uuid, str):
            if object_id_or_uuid.isdigit():
                return base_query.filter(DBRegistryObject.id == int(object_id_or_uuid))
            return base_query.filter(DBRegistryObject.uuid == object_id_or_uuid)
        else:
            raise ValueError("Invalid identifier type")

    def register_object(self, obj_data: RegistryObjectCreate) -> DBRegistryObject:
        """Register a new object in the database scoped to the workspace."""
    ...
    def list_objects(self, filters: Optional[dict] = None) -> List[DBRegistryObject]:
        """List objects, scoped to the workspace, optionally filtering by specific fields."""
        query = self.db.query(DBRegistryObject).filter(DBRegistryObject.workspace_id == self.workspace_id)
        if filters:
    ...
    def get_statistics(self) -> dict:
        """Calculate and return registry statistics scoped to the workspace."""
        total = self.db.query(DBRegistryObject).filter(DBRegistryObject.workspace_id == self.workspace_id).count()
        
        by_type_query = self.db.query(
            DBRegistryObject.object_type, 
            func.count(DBRegistryObject.id)
        ).filter(DBRegistryObject.workspace_id == self.workspace_id).group_by(DBRegistryObject.object_type).all()
        by_type = {r[0]: r[1] for r in by_type_query}
        
        by_source_query = self.db.query(
            DBRegistryObject.source_system, 
            func.count(DBRegistryObject.id)
        ).filter(DBRegistryObject.workspace_id == self.workspace_id).group_by(DBRegistryObject.source_system).all()
    ...
    def find_by_hash(self, content_hash: str) -> Optional[DBRegistryObject]:
        """Find a registry object by its content hash within the workspace."""
        if not content_hash:
            return None
        return self.db.query(DBRegistryObject).filter(
            DBRegistryObject.workspace_id == self.workspace_id,
            DBRegistryObject.content_hash == content_hash
        ).first()

    def record_sync_run(self, sync_run_data: SyncRunCreate) -> DBSyncRun:
        """Record a sync run record in the database scoped to the workspace."""
        data = sync_run_data.model_dump()
        data["workspace_id"] = self.workspace_id
        if hasattr(self, "source_id") and self.source_id is not None:
            data["source_id"] = self.source_id

        if getattr(self, "run_uuid", None):
            db_run = self.db.query(DBSyncRun).filter(DBSyncRun.uuid == self.run_uuid).first()
            if db_run:
                for key, val in data.items():
                    setattr(db_run, key, val)
                self.db.commit()
                self.db.refresh(db_run)
                return db_run

        db_run = DBSyncRun(**data)
        self.db.add(db_run)
        self.db.commit()
        self.db.refresh(db_run)
        return db_run
    ...
    def list_sync_runs(self) -> List[DBSyncRun]:
        """List all sync runs within the workspace, ordered by started_at descending."""
        return self.db.query(DBSyncRun).filter(DBSyncRun.workspace_id == self.workspace_id).order_by(DBSyncRun.started_at.desc()).all()
    ...
    def search_objects(
        ...
        """Search active registry objects using case-insensitive LIKE matching on extracted keywords in title, description, or location."""
        ...
        if query and query.strip():
            ...
            db_query = db_query.filter(or_(*kw_conditions))
    ...
    def get_object_details(self, id_or_uuid: Union[int, str]) -> Optional[dict]:
        ...
        from deepcore2.storage.sqlite.models import RegistryRelationship as DBRegistryRelationship, RegistrySignal as DBRegistrySignal
        ...
        for rel, target in rel_query:
            ...
            nav_hint = f"/concepts/{target.title}" if target.object_type == "concept" else f"/memories/{target.uuid}"
    ...
    def get_dashboard_counts(self) -> dict:
        ...
        from deepcore2.storage.sqlite.models import RegistryRelationship as DBRegistryRelationship
